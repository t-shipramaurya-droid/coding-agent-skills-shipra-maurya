...
const DEFAULT_ENGINE = path.join(ENGINE_DIR, "target/debug/fraud-score");

export function ensureEngineBuilt(enginePath = DEFAULT_ENGINE) {
  if (enginePath !== DEFAULT_ENGINE) {
    return enginePath;
  }
...
export function scoreWithRustEngine(transaction, enginePath = DEFAULT_ENGINE) {
  const resolvedEngine = ensureEngineBuilt(enginePath);
  return runEngine(resolvedEngine, JSON.stringify(toEnginePayload(transaction)));
}
